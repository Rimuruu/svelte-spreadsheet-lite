...
import { onMount } from 'svelte';
    ...
    $: selectionTransform=()=> {
      return `translate(${positionLeft(selectionCount.c)},  ${
      selectionCount.r * 24})`;
    }
    ...
    $: selectionCount= {
        ...
        r:
          selection.r <= selection.sr
            ? selection.r
            : selection.sr,
        c:
          selection.c <= selection.sc
            ? selection.c
            : selection.sc,
    ...
    $: editorStyleObj = ()=> {
      return " left: "+positionLeft(selection.c)+"px;top: "+(selection.r * 24) + "px;width: "+selectionSize.w + "px";
    }
    ...
    let headerResizeMove = (e)=> {
      ...
      var headerMouseX = e.clientX - headerRect.left+5;
      if (headerResizeAt >= 0) {
        const updateWidth =
          headerMouseX - positionLeft(headerResizeAt);
        header[headerResizeAt].width =
    ...
    function moveInputCaretToEnd() {
      ...
      setSelectionRange(editingText.length, editingText.length);
    }
    function fixScroll() {
      ...
      if (
        el.scrollLeft <
        positionLeft(selection.c) - el.clientWidth
      ) {
...
<template>
  <div class="grid"  on:mouseup={onMouseUpSvg} on:mousemove={headerResizeMove} bind:this={el}>
    <svg width={positionLeft(data.length +1) +1} height="24">
    {#each header as col,ci}
      <g  key={ci} transform={translateCol(ci)} >
        <rect class="col-header" x="0" y="0" width={col.width} height={rowHeight} on:mousedown={()=>{startColumnSelect(ci)}} on:mousemove={()=>{changeColumnSelect(ci)}} on:mouseup={()=>{endColumnSelect}}>
        </rect>
        <text class="col-header__text" text-anchor="middle" x={col.width / 2} y="12" width={col.width} height={rowHeight}>{col.name}</text>
        <rect class={"col-header__resize" + ( ci === headerResizeAt?'active':"")} x={col.width - 5} y="0" width="5" height={rowHeight} on:mousedown={()=>{headerResizeStart(ci)}}></rect>
      </g>
    ...
    <div ref="wrapper" style="height: 400px; overflow: scroll; position:relative;">
      <svg width={positionLeft(data.length + 1) + 1} height={data.length * 24} >
       {#each data as row, ri}
        ...
        {#each row as col,ci}
          <g key="ci" transform={translateCol(ci)} on:mousedown={()=>{onMouseDownCell(ci, ri)}} on:mousemove={()=>{onMouseMoveCell(ci, ri)}}>
      ...
      <div class="editor__frame" style={editorStyleObj()}>
        <input ref="hiddenInput" bind:this={input}  on:mousedown={()=>{onMouseDownCell(selection.c, selection.r)}} class={"editor__textarea " +(editing?'editor--visible':"") } bind:value={editingText} on:blur={()=>{onBlur}} />
      </div>
    </div>-
  </div>
</template>